...
parser.add_argument('--save', type=str, default='ADV', help='experiment name')
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    #   delta[:, i, :, :].uniform_(-epsilon[i][0][0], epsilon[0][0][0])
    ...
    # print('grad', input.requires_grad, grad)
    # print('grad', grad.type)
    # print(delta.type(), alpha.type(), grad.type())
    # print('input', input[0].max())
    # print('alpha', alpha * torch.sign(grad))
    
    delta = clamp(alpha * torch.sign(grad), -epsilon, epsilon)
    # print('delta 1', delta.max())
